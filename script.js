document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('files.json')
        ...
        .then(data => {
            // Get base64-encoded folder name from URL (e.g., ?cHlpbnN0YWxsZXI= for "pyinstaller")
            const rawQuery = window.location.search.substring(1);
            let decodedQuery = '';
            try {
                decodedQuery = atob(rawQuery).toLowerCase();
            } catch (e) {
                console.warn('Invalid base64 in URL, falling back to first folder');
            }
            
            // Find the matching folder in the array, or default to the first one
            let selectedFolder = data.find(folder => folder['folder-name'].toLowerCase() === decodedQuery);
